...
RSpec.describe Api::V1::CardSidesController, type: :controller do
  ...
  context 'when not signed in' do
    describe '#show' do
      it '401s' do
        get :show, params: { id: card_side.id }
    ...
    describe '#update' do
      it '401s' do
        put :update, params: { id: card_side.id, card_side: { image: 'bah' } }
        expect(response.status).to eq(401)
        expect(json).to include('errors')
      end
    end
  ...
  context 'logged in' do
    ...
    describe '#show' do
      ...
      context 'when card_side owned by user' do
        context 'if side exists' do
          before(:each ) { get :show, params: { id: card_side.id } }
    ...
    describe '#update' do
      context 'if side exists' do
        it 'succeeds' do
          put :update, params: { id: card_side.id, card_side: { image: 'New Image' } }
          expect(response.status).to eq(200)
        end
      ...
      context 'if side does not exist' do
        it '404s' do
          put :update, params: { id: -1, card_side: { image: 'New Image' } }
