class CardTemplatesController < AuthenticatedController
  ...
  def create
    ...
    @card_template.discount_pct = 10
    @card_template.discount_loc = "10.00,65.00"
  ...
  def discount_confirm
    puts "discount confirm!"
    @card_template = CardTemplate.find(params[:id])
    @card_template.update_attributes(discount_params)
  ...
  def update_style
    if card_params[:style].downcase.include?("thank you")
      ...
      @card_template.update(style: "discount")
    end
  end
